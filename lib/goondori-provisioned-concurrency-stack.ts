...
export class GoondoriProvisionedConcurrencyStack extends cdk.Stack {
  constructor(scope: Construct, id: string, props?: cdk.StackProps) {
    ...
    const graphqlLambda = lambda.Function.fromFunctionName(this, `GraphqlLambda${NODE_ENV_PASCAL}`, GRAPHQL_LAMBDA_NAME);
    ...
    const scalingTarget = new applicationautoscaling.ScalableTarget(this, `ScalingTarget${NODE_ENV_PASCAL}`, {
    ...
    scalingTarget.scaleOnSchedule(`DinnerTimeScaleOut${NODE_ENV_PASCAL}`, {
      schedule: applicationautoscaling.Schedule.cron({minute: dinnerStartTime.getUTCMinutes().toString(), hour: dinnerStartTime.getUTCHours().toString()}),
      maxCapacity: 10,
      minCapacity: 9,
    })
